...
size_t decodeHexChars(unsigned char *bytes, size_t numBytes, const char *hex) {
  ...
  /* We can't sscanf directly into the byte array since %02x expects a pointer
   * to int, not a pointer to unsigned char.  Also, since we expect an unbroken
   * string of hex characters, we check for that before calling sscanf;
   * otherwise we would get a framing error if there's whitespace in the input
   * string. */
  while (numWritten < numBytes && isxdigit(hex[2 * numWritten]) &&
         isxdigit(hex[2 * numWritten + 1])) {
    int byte;
 ...
 * parameter specifies the length of the output buffer.  The number of bytes
 * actually written to the output buffer is returned. */
size_t decodeHexWChars(unsigned char *bytes, size_t numBytes,
                       const wchar_t *hex) {
  size_t numWritten = 0;

  /* We can't swscanf directly into the byte array since %02x expects a pointer
   * to int, not a pointer to unsigned char.  Also, since we expect an unbroken
   * string of hex characters, we check for that before calling swscanf;
   * otherwise we would get a framing error if there's whitespace in the input
   * string. */
